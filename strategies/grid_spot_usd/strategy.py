...
if df_order.empty or len(df_order.loc[df_order["side"] == "buy"]) == 0 or len(df_order.loc[df_order["side"] == "sell"]) == 0:
    ...
    for sell in grid_sell:
        ...
        ftx.place_limit_order(symbol=symbol, side='sell', amount=coin1_balance/len(grid_sell), price=sell)

elif total_orders == len(df_order):
    print("no new orders")

else:
    buy_order_to_create = last_data["number_of_sell_orders"] - len(df_order.loc[df_order["side"] == "sell"])
    sell_order_to_create = last_data["number_of_buy_orders"] - len(df_order.loc[df_order["side"] == "buy"])
    ...
    for i in range(buy_order_to_create):
        ...
        ftx.place_limit_order(symbol=symbol, side='buy', amount=(coin2_balance/current_price)/buy_order_to_create, price=current_price - diff_buy*(i+1))
    for i in range(sell_order_to_create):
        # print("sell",current_price + diff_sell*(i+1))
        ftx.place_limit_order(symbol=symbol, side='sell', amount=coin1_balance/sell_order_to_create, price=current_price + diff_sell*(i+1))
